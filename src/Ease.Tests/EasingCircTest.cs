namespace Ease.Tests
{
	public class EasingCircTest
	{
		...
		{
			IEasingFunction function = Easing.Circ.ToFunction(EasingMode.In);
		...
		{
			IEasingFunction function = Easing.Circ.ToFunction(EasingMode.Out);
			double result = function.Interpolate(input);
			Assert.Equal(expected, result, precision);
		}
		...
		{
			IEasingFunction function = Easing.Circ.ToFunction(EasingMode.InOut);
			double result = function.Interpolate(input);
			Assert.Equal(expected, result, precision);
		}
	}
