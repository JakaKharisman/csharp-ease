...
namespace Ease.Tests
{
	public class EasingBounceTest
	{
		...
		{
			IEasingFunction function = Easing.Bounce.ToFunction(EasingMode.In);
		...
		{
			IEasingFunction function = Easing.Bounce.ToFunction(EasingMode.Out);
			double result = function.Interpolate(input);
			Assert.Equal(expected, result, precision);
		}
		...
		{
			IEasingFunction function = Easing.Bounce.ToFunction(EasingMode.InOut);
			double result = function.Interpolate(input);
			Assert.Equal(expected, result, precision);
		}
	}
