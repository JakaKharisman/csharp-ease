namespace Ease.Tests
{
	public class EasingQuadTest
	{
		...
		{
			IEasingFunction function = Easing.Quad.ToFunction(EasingMode.Out);
			double result = function.Interpolate(input);
			Assert.Equal(expected, result, precision);
		}
		...
		{
			IEasingFunction function = Easing.Quad.ToFunction(EasingMode.InOut);
			double result = function.Interpolate(input);
			Assert.Equal(expected, result, precision);
		}
	}
