...
namespace Ease.Tests
{
	public class EasingCubicTest
	{
		...
		{
			IEasingFunction function = Easing.Cubic.ToFunction(EasingMode.In);
		...
		{
			IEasingFunction function = Easing.Cubic.ToFunction(EasingMode.Out);
			double result = function.Interpolate(input);
			Assert.Equal(expected, result, precision);
		}
		...
		{
			IEasingFunction function = Easing.Cubic.ToFunction(EasingMode.InOut);
			double result = function.Interpolate(input);
			Assert.Equal(expected, result, precision);
		}
	}
