...
namespace Ease.Tests
{
	public class EasingSineTest
	{
		...
		{
			IEasingFunction function = Easing.Sine.ToFunction(EasingMode.In);
		...
		{
			IEasingFunction function = Easing.Sine.ToFunction(EasingMode.Out);
			double result = function.Interpolate(input);
			Assert.Equal(expected, result, precision);
		}
		...
		{
			IEasingFunction function = Easing.Sine.ToFunction(EasingMode.InOut);
			double result = function.Interpolate(input);
			Assert.Equal(expected, result, precision);
		}
	}
