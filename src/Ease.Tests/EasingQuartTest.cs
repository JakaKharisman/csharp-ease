...
namespace Ease.Tests
{
	public class EasingQuartTest
	{
		...
		{
			IEasingFunction function = Easing.Quart.ToFunction(EasingMode.In);
		...
		{
			IEasingFunction function = Easing.Quart.ToFunction(EasingMode.Out);
			double result = function.Interpolate(input);
			Assert.Equal(expected, result, precision);
		}
		...
		{
			IEasingFunction function = Easing.Quart.ToFunction(EasingMode.InOut);
			double result = function.Interpolate(input);
			Assert.Equal(expected, result, precision);
		}
	}
